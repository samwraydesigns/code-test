...
gulp.task('html', () => {
  return gulp
    .src(`${paths.templates}pages/*.html`)
    .pipe(
      include({
        prefix: '@@',
        basepath: '@file'
      ...
      sprite({
        shape: {
          ...
          transform: [
            {
              svgo: {
                plugins: [{ convertStyleToAttrs: false }]
              }
            }
          ]
        },
...
gulp.task('concatJSLibs', () => {
  return gulp
    .src([`${paths.srcJSLibs}*.js`, `!${paths.srcJSLibs}jquery-3.2.1.min.js`])
    .pipe(concat('libs.js'))
    .pipe(gulp.dest(paths.buildJS))
    .pipe(browserSync.stream());
});
...
gulp.task('watchAll', () => {
  ...
  watch(
    [`${paths.srcJSLibs}jquery-3.2.1.min.js`, `${paths.srcJS}polyfills/*.js`],
    batch((events, done) => {
...
gulp.task('default', (callback) => {
  ...
  gulp.series('setVars', 'build', 'html', gulp.parallel('serve', 'watchAll'))(
    callback
  );
});

gulp.task('copyBuild', () => {
  return gulp.src('./build/**/*.*').pipe(gulp.dest('./html/build'));
});
gulp.task('copyImages', () => {
  return gulp.src('./images/**/*.*').pipe(gulp.dest('./html/images'));
});
gulp.task('copyAssets', () => {
  return gulp.src('./assets/**/*.*').pipe(gulp.dest('./html/assets'));
});
